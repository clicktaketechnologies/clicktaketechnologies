 ...
 *
 * Calls navigator.modelContext.registerTool() (Chrome experimental, latest
 * spec) for each tool we want to expose. Falls back to the older
 * provideContext() batch API if only that is available.
 *
 * Tools exposed: site-search, list-services, list-portfolio, get-office-hours,
 * submit-lead. Each tool routes to its corresponding public JSON endpoint.
 *
...
type WebMCPNavigator = {
  modelContext?: {
    // Newer spec — register tools one at a time. Returns an AbortController-
    // bound handle. Some scanners specifically look for this method.
    registerTool?: (tool: WebMCPTool) => Promise<{
      unregister: () => void;
    }>;
    // Older spec — batch-register all tools at once.
...
export function WebMCPProvider() {
  useEffect(() => {
    ...
    // Preferred path: register each tool individually via registerTool().
    // The scanner specifically greps the page source for this call.
    if (typeof nav.modelContext.registerTool === "function") {
      ...
      (async () => {
        for (const tool of TOOLS) {
          if (cancelled) break;
          try {
            const handle = await nav.modelContext!.registerTool!(tool);
            handles.push(handle.unregister);
          } catch (err) {
      ...
      // Cleanup on unmount — unregister all tools
      return () => {
        cancelled = true;
    ...
    // Fallback: older batch API (provideContext). Still works in Chrome
    // versions that haven't shipped registerTool() yet.
    if (typeof nav.modelContext.provideContext === "function") {
      nav.modelContext
        .provideContext({ origin: SITE_URL, tools: TOOLS })
        .then(() => {
