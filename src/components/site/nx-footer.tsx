...
export function NxFooter() {
  return (
    <footer className="relative bg-[***REMOVED***0A1124] text-white overflow-hidden">
      {/* Top decorative gradient line */}
      <div className="h-px bg-gradient-to-r from-transparent via-[***REMOVED***FF6B35]/50 to-transparent" />
      {/* Subtle dot grid */}
              ...
              {SITE.socials.slice(0, 4).map((s) => {
                ...
                return (
                  <a
                    ...
                    aria-label={s.name}
                    className="h-10 w-10 rounded-lg bg-white/5 border border-white/10 grid place-items-center text-white/70 hover:bg-[***REMOVED***FF6B35] hover:text-white hover:border-[***REMOVED***FF6B35] transition"
                  >
            ...
            <ul className="space-y-3">
              {FOOTER_LINKS.services.map((l) => (
                <li key={l.label}>
                  <Link
                    href={l.href}
                    className="text-sm text-white/70 hover:text-[***REMOVED***FF8A5C] transition"
                  >
                    {l.label}
                  </Link>
                </li>
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-4">
              <li>
                ...
                >
                  <Mail className="h-4 w-4 text-[***REMOVED***FF8A5C] mt-0.5 shrink-0" />
              ...
              {SITE.phones.map((p) => (
                <li key={p.href}>
                  <a
                    ...
                    className="group flex items-start gap-3 text-sm text-white/70 hover:text-white transition"
                  >
                    <Phone className="h-4 w-4 text-[***REMOVED***FF8A5C] mt-0.5 shrink-0" />
                    <span>
              ...
              <li>
                <div className="flex items-start gap-3 text-sm text-white/70">
                  <MapPin className="h-4 w-4 text-[***REMOVED***FF8A5C] mt-0.5 shrink-0" />
                  <span>
