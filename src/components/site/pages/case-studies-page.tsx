...
function CaseStudyCard({ cs }: { cs: CaseStudy }) {
  ...
  return (
    <TiltCard
      className="group/tilt flex flex-col rounded-2xl border border-border bg-[***REMOVED***14141A] overflow-hidden hover:border-[***REMOVED***EC4899]/40 transition-colors"
      glow={true}
      shine={true}
      maxTilt={8}
    >
      <Link href={`/case-studies/${cs.slug}`} className="contents">
        <div className="h-40 bg-[***REMOVED***1A1A22] border-b border-[***REMOVED***EC4899]/20 relative overflow-hidden">
          <div className="absolute inset-0 bg-black/10" />
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            <span className="inline-flex items-center rounded-[6px] bg-[***REMOVED***EC4899] px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-white">
              {cs.industry}
            </span>
        ...
        <div className="flex-1 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {cs.services.slice(0, 3).map((s) => (
              <span
                key={s}
                className="inline-flex items-center rounded-[6px] border border-border bg-[***REMOVED***0F0F14] px-2 py-0.5 text-[10px] font-medium"
              >
                {SERVICE_LABELS[s] || s}
              </span>
            ))}
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {cs.metrics.slice(0, 2).map((m) => (
              <div
                key={m.label}
                className="rounded-lg border border-border bg-[***REMOVED***0F0F14] px-2.5 py-1.5"
              >
            ...
            <button
              ...
              className={`inline-flex items-center rounded-[8px] px-3 py-1.5 text-xs font-semibold transition ${
                filter === "all"
                  ...
                  : "border border-border bg-[***REMOVED***14141A] text-foreground hover:border-[***REMOVED***EC4899]/40"
              }`}
            >
          ...
          >
            <div>
              <div className="flex items-center gap-2 text-lg font-bold">
                <Sparkles className="h-5 w-5 text-[***REMOVED***EC4899] shrink-0" />
            ...
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 rounded-[8px] bg-[***REMOVED***EC4899] px-5 py-2.5 text-sm font-semibold text-white hover:-translate-y-0.5 transition shrink-0"
            >
    ...
    <NxPageLayout>
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center rounded-[6px] border border-[***REMOVED***EC4899]/25 bg-[***REMOVED***EC4899]/8 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[***REMOVED***EC4899]">
                {cs.industry}
              </span>
          ...
          {/* Hero image */}
          <div className="mt-8 h-48 sm:h-72 rounded-2xl bg-[***REMOVED***1A1A22] border border-[***REMOVED***EC4899]/15 relative overflow-hidden flex items-end justify-end p-4">
          ...
          <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3">
            {cs.metrics.map((m) => (
              <div key={m.label} className="rounded-xl border border-border bg-[***REMOVED***14141A] p-4">
          ...
          >
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {cs.tech.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center rounded-[6px] border border-[***REMOVED***EC4899]/25 bg-[***REMOVED***EC4899]/8 px-3 py-1.5 text-xs font-medium text-[***REMOVED***F5F5F0]"
                >
                  {t}
                </span>
              ))}
          ...
          >
            ...
            {cs.live_url && (
              <a
                ...
                className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[***REMOVED***EC4899] hover:underline"
              >
                ...
                {related.map((r) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group rounded-xl border border-border bg-[***REMOVED***14141A] p-4 hover:border-[***REMOVED***EC4899]/40 transition"
                    >
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-[10px] uppercase tracking-widest text-[***REMOVED***EC4899]">
                          {r.industry}
                        </span>
