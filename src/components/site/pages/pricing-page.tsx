...
const PLAN_GRADIENT: Record<string, string> = {
  ...
  growth: "bg-[***REMOVED***EC4899]/15 text-[***REMOVED***EC4899]",
  scale: "bg-[***REMOVED***EC4899]/15 text-[***REMOVED***EC4899]",
  custom: "from-amber-500 to-orange-500",
};

function PricingCard({ plan }: { plan: PricingPlan }) {
  const Icon = PLAN_ICON[plan.slug] || Sparkles;
  const gradient = PLAN_GRADIENT[plan.slug] || "bg-[***REMOVED***EC4899]/15 text-[***REMOVED***EC4899]";
  ...
  return (
    <TiltCard
      className={`relative rounded-2xl border p-6 sm:p-7 flex flex-col ${
        plan.highlight
          ? "border-[***REMOVED***EC4899]/40 bg-[***REMOVED***14141A] shadow-xl"
          : "border-border bg-[***REMOVED***14141A]"
      }`}
      glow={plan.highlight}
      shine={true}
      maxTilt={8}
    >
      {plan.highlight && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-[6px] bg-[***REMOVED***EC4899] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white">
          Most Popular
        </div>
      )}
      ...
      <ul className="mt-5 space-y-2 flex-1">
        {plan.features.map((f) => (
          <li key={f} className="flex items-start gap-2 text-sm">
            <Check className="h-4 w-4 shrink-0 text-[***REMOVED***EC4899] mt-0.5" />
      ...
      <Link
        ...
        className={`mt-6 inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold transition shrink-0 ${
          plan.highlight
            ? `bg-[***REMOVED***EC4899] text-white hover:-translate-y-0.5`
...
{/* Per-page 3D Story Scene — pricing variant. Sits behind hero content. */}
        <NxStoryScene variant="pricing" />
                <div className="pointer-events-none absolute right-0 top-24 lg:top-32 xl:top-40 z-[5] hidden lg:block" aria-hidden="true">
          <Nx3DCharacter variant="pricing" size="md" />
        </div>
        ...
        <Nx3DScene density="low" />
          ...
          >
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <div
                  key={item}
                  className="flex items-start gap-3 rounded-xl border border-border bg-[***REMOVED***14141A] p-4"
                >
          ...
          >
            <div className="flex items-center gap-2 mb-4 sm:mb-6">
              <HelpCircle className="h-5 w-5 text-[***REMOVED***EC4899] shrink-0" />
            ...
            <div className="space-y-3">
              {FAQS.map((faq) => (
                <details
                  key={faq.q}
                  className="group rounded-xl border border-border bg-[***REMOVED***14141A] p-4 [&_summary]:cursor-pointer"
                >
                  <summary className="flex items-center justify-between gap-3 text-sm font-semibold list-none">
                    <span>{faq.q}</span>
                    <ChevronRight className="h-4 w-4 shrink-0 transition-transform group-open:rotate-90 text-[***REMOVED***EC4899]" />
                  </summary>
