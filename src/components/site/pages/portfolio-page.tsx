...
{/* Per-page 3D Story Scene — portfolio variant. Sits behind hero content. */}
        ...
        <Nx3DScene density="low" />
          ...
          <div className="mt-8 sm:mt-10 grid gap-4 sm:grid-cols-3">
            {Object.entries(CATEGORY_COUNT).map(([cat, count], i) => (
              <motion.div
                ...
                className="rounded-2xl border ct-divider bg-[***REMOVED***14141A] p-5"
              >
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mt-12 rounded-2xl border ct-divider bg-[***REMOVED***14141A] p-5 sm:p-6 text-center"
          >
            <div className="inline-flex items-center gap-2 text-sm font-semibold">
              <Sparkles className="h-4 w-4 text-[***REMOVED***EC4899]" />
...
function PortfolioCard({ p, index }: { p: ClientPortfolio; index: number }) {
  ...
  const gradient = CATEGORY_GRADIENT[p.category] || "bg-[***REMOVED***EC4899]/15 text-[***REMOVED***EC4899]";
  ...
  return (
    ...
    >
      <TiltCard
        className="group/tilt relative overflow-hidden rounded-[24px] border ct-divider bg-[***REMOVED***14141A] hover:shadow-cyan-500/10"
        glow={true}
        shine={false}
        maxTilt={6}
      >
        ...
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          <div className="absolute left-4 bottom-4 inline-flex items-center gap-1.5 rounded-[6px] bg-black/70 px-2.5 py-1 text-[10px] font-semibold text-white/90">
            <span className={`h-1.5 w-1.5 rounded-full bg-[***REMOVED***EC4899]`} />
            {p.category}
          </div>
        ...
        <div className="p-5">
          ...
          <div className="mt-4 flex items-center justify-between gap-3 border-t ct-divider pt-3">
            ...
            <a
              ...
              className="group/btn inline-flex items-center gap-1.5 rounded-[8px] bg-[***REMOVED***EC4899] px-3.5 py-1.5 text-[11px] font-semibold text-white transition-all duration-300 hover:-translate-y-0.5"
            >
              Visit live site
