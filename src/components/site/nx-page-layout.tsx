...
export function NxPageHero({
  ...
  return (
    <section className="relative overflow-hidden nx-hero-bg pt-28 sm:pt-32 lg:pt-36 pb-16 lg:pb-20">
      ...
      <div className="absolute top-10 -left-20 h-72 w-72 rounded-full bg-[***REMOVED***FF6B35]/15 blur-3xl nx-orb pointer-events-none" />
      <div
        className="absolute bottom-0 right-10 h-80 w-80 rounded-full bg-[***REMOVED***3B82F6]/20 blur-3xl nx-orb pointer-events-none"
      ...
      <div className="relative mx-auto max-w-7xl px-4 lg:px-8">
        ...
        {eyebrow && (
          ...
          >
            <span className="h-1.5 w-1.5 rounded-full bg-[***REMOVED***FF6B35] animate-pulse" />
            {eyebrow}
          </motion.div>
        )}
          ...
          >
            {stats.map((s, i) => (
              <div key={i} className="text-center sm:text-left">
                <div className="nx-stat-num text-3xl sm:text-4xl lg:text-5xl text-[***REMOVED***FF6B35]">
                  {s.value}
                </div>
...
export function NxButton({ href, children, variant = "orange", className = "" }: NxButtonProps) {
  ...
  if (variant === "dark") {
    return (
      <Link
        href={href}
        className="inline-flex items-center justify-center gap-2 px-6 py-3 text-sm sm:text-base font-bold rounded-full bg-[***REMOVED***0A1124] text-white hover:bg-[***REMOVED***1E2F5C] transition shadow-lg ${className}"
      >
        {children}
        <ArrowRight className="h-4 w-4 nx-arrow" />
      </Link>
