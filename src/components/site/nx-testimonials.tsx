        ...
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {TESTIMONIALS.map((t, i) => (
            <motion.div
              ...
              className="relative p-6 sm:p-7 rounded-2xl border nx-bd nx-surface hover:shadow-xl hover:-translate-y-1 hover:border-[***REMOVED***FF53A9]/40 transition-all flex flex-col"
            >
              {/* Quote icon + rating */}
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: t.rating }).map((_, idx) => (
                    <Star
                      key={idx}
                      className="h-3.5 w-3.5 fill-[***REMOVED***FF53A9] text-[***REMOVED***FF53A9]"
                    />
                  ))}
                </div>
              </div>
