...
const TESTIMONIALS = [
  ...
  {
    quote:
      "The mobile app they shipped hit ***REMOVED***4 in App Store productivity within 2 weeks. The team thinks like product people, not just code monkeys.",
    name: "Priya Sharma",
...
export function NxTestimonials() {
  return (
    <section className="py-24 sm:py-32 bg-[***REMOVED***F8FAFC]">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <motion.h2
            ...
            className="mt-4 text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight text-[***REMOVED***0A1124]"
          >
            Rated{" "}
          ...
          <motion.div
            ...
            className="mt-5 inline-flex items-center gap-2 text-sm text-[***REMOVED***94A3B8]"
          >
            <div className="flex items-center gap-0.5">
              {[1, 2, 3, 4, 5].map((i) => (
                <Star key={i} className="h-4 w-4 fill-[***REMOVED***FF6B35] text-[***REMOVED***FF6B35]" />
              ))}
            </div>
        ...
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {TESTIMONIALS.map((t, i) => (
            <motion.div
              ...
              className="relative p-6 sm:p-7 rounded-2xl border border-[***REMOVED***E2E8F0] bg-white hover:shadow-xl hover:-translate-y-1 hover:border-[***REMOVED***FF6B35]/40 transition-all flex flex-col"
            >
              {/* Quote icon + rating */}
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: t.rating }).map((_, idx) => (
                    <Star
                      key={idx}
                      className="h-3.5 w-3.5 fill-[***REMOVED***FF6B35] text-[***REMOVED***FF6B35]"
                    />
                  ))}
                </div>
              </div>
              ...
              {/* Quote */}
              <p className="text-sm sm:text-[15px] text-[***REMOVED***0A1124] leading-relaxed flex-1">
                "{t.quote}"
              </p>

              {/* Author */}
              <div className="mt-6 pt-5 border-t border-[***REMOVED***E2E8F0] flex items-center gap-3">
                <div
                ...
                <div>
                  <div className="font-bold text-sm text-[***REMOVED***0A1124]">{t.name}</div>
                  <div className="text-xs text-[***REMOVED***94A3B8] mt-0.5">
                    {t.role} · {t.location}
                  </div>
                </div>
              </div>
