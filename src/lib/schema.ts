...
export const cmsThemes = pgTable("cms_themes", {
  ...
  primary: text("primary").default("***REMOVED***136DFF"),
  accent: text("accent").default("***REMOVED***FF53A9"),
...
export const adminRoles = pgTable("admin_roles", {
  ...
  name: text("name").notNull().unique(),
  description: text("description").default(""),
  color: text("color").default("***REMOVED***136DFF"),
...
export const services = pgTable(
  "services",
  {
    ...
    // Phase 3 ***REMOVED***2 — full 12-section deep-dive content generated by the
    // Master Copywriting prompt (In-Admin LLM Page Creation). Stored as a
    // JSON-stringified DeepDiveContent blob. Empty when the page was
    // authored manually (the legacy 32 deep-dive pages keep their content
    // in /src/content/deep-dive/<slug>.ts instead).
    deepDive: text("deep_dive").default("{}"),
    displayOrder: integer("display_order").default(0),
    isPublished: boolean("is_published").default(true),
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
...
// ─── A/B Experiments (Phase 3 ***REMOVED***3) ───────────────────────────────────────────
