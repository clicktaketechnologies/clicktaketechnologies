...
//      password matches the default `***REDACTED_ADMIN_PASSWORD***`.
//   2. Recover: ?action=reset — forces the admin user's password back to
//      the SUPERADMIN_PASSWORD env var (or default `***REDACTED_ADMIN_PASSWORD***`).
//
// Intended workflow:
...
//   - Login with admin@clicktaketech.com / ***REDACTED_ADMIN_PASSWORD***
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (action === 'reset') {
      ...
      const newPassword = process.env.SUPERADMIN_PASSWORD || '***REDACTED_ADMIN_PASSWORD***'
