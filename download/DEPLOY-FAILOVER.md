***REMOVED*** Active/Passive Failover Setup — cPanel + Vercel + Cloudflare Worker
...
***REMOVED******REMOVED*** Step 1 — Deploy to cPanel (primary)
...
***REMOVED******REMOVED*** Step 2 — Deploy to Vercel (secondary)
...
***REMOVED******REMOVED*** Step 3 — Deploy the failover Worker

The Worker code is already in your repo at `workers/cloudflare-failover/`.

***REMOVED******REMOVED******REMOVED*** 3.1 — Edit `wrangler.toml`
...
```toml
[vars]
PRIMARY_BACKEND = "https://<your-cpanel-url>"        ***REMOVED*** e.g. https://server.a2hosting.com/~clicktake
SECONDARY_BACKEND = "https://<your-vercel-url>"      ***REMOVED*** e.g. https://clicktaketechnologies-abc.vercel.app
...
***REMOVED******REMOVED******REMOVED*** 3.2 — Get your Cloudflare API token
...
***REMOVED******REMOVED******REMOVED*** 3.3 — Deploy the Worker
...
***REMOVED*** Should return {"status":"ok"} with header: x-backend: primary
```

---

***REMOVED******REMOVED*** Step 4 — Route `clicktaketech.com` through the Worker
...
***REMOVED******REMOVED******REMOVED*** 4.1 — Remove the old Worker's custom domain
...
***REMOVED******REMOVED******REMOVED*** 4.2 — Add custom domain to the new Worker
...
***REMOVED******REMOVED******REMOVED*** 4.3 — Verify DNS
...
***REMOVED******REMOVED*** Step 5 — Test the failover

***REMOVED******REMOVED******REMOVED*** 5.1 — Normal operation

```bash
curl -i https://clicktaketech.com/api/health
***REMOVED*** Expected: 200 OK, header x-backend: primary
```

Open `https://clicktaketech.com/admin/login` — should work normally.

***REMOVED******REMOVED******REMOVED*** 5.2 — Simulate cPanel failure
...
```bash
cd /home/z/my-project/workers/cloudflare-failover
***REMOVED*** Edit wrangler.toml — change PRIMARY_BACKEND to https://broken.example.invalid
CLOUDFLARE_API_TOKEN=cfut_... bunx wrangler deploy
```

Now test:
```bash
***REMOVED*** First 2 requests: try primary, fail, fall through to secondary
curl -i https://clicktaketech.com/api/health
***REMOVED*** Expected: 200 OK, header x-backend: secondary

***REMOVED*** Subsequent requests: circuit is open, go straight to secondary (fast)
curl -i https://clicktaketech.com/api/health
***REMOVED*** Expected: 200 OK, header x-backend: secondary, faster
```

Visit `https://clicktaketech.com` in your browser — should still load, now
served by Vercel.

***REMOVED******REMOVED******REMOVED*** 5.3 — Simulate cPanel recovery
...
```bash
curl -i https://clicktaketech.com/api/health
***REMOVED*** After ~60s, expected: x-backend: primary
```

---

***REMOVED******REMOVED*** Step 6 — Set up monitoring alerts (optional but recommended)
...
When you get an alert for monitor ***REMOVED***2 (cPanel directly down) but monitor ***REMOVED***1
still passes, failover is working as designed. When monitor ***REMOVED***1 also fails,
both backends are down — that's a real emergency.

---

***REMOVED******REMOVED*** Trade-offs vs. paid Cloudflare Load Balancing
...
***REMOVED******REMOVED******REMOVED*** "502 Both backends unreachable"
...
***REMOVED******REMOVED******REMOVED*** Failover doesn't trigger
...
***REMOVED******REMOVED******REMOVED*** Sessions don't persist across failover
...
***REMOVED******REMOVED******REMOVED*** `x-backend: secondary` even when cPanel is up
...
***REMOVED******REMOVED******REMOVED*** Worker URL works but custom domain returns old site
...
***REMOVED******REMOVED******REMOVED*** CORS errors after failover
