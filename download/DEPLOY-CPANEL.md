...
| Command | `curl -s -X POST -H "Authorization: Bearer ***REDACTED_CRON_SECRET***" https://clicktaketech.com/api/cron/provider-health > /dev/null 2>&1` |

Replace `***REDACTED_CRON_SECRET***` with your actual `CRON_SECRET` value.
