...
SKIP_FILES = {"about-page.tsx", "services-page.tsx"}  ***REMOVED*** Already migrated manually
...
def migrate_file(path: Path) -> tuple[bool, str]:
    ...
    ***REMOVED*** 1. Remove old imports (one per line, possibly with varying whitespace)
    old_imports = [
    ...
    ***REMOVED*** 2. Add NxPageLayout import (after the last `from "lucide-react"` or
    ***REMOVED***    `from "next/link"` line — whichever comes first in the file)
    nx_import = (
        'import {\n'
        '  NxPageLayout, NxPageHero, NxPageSection, NxSectionHeader, NxButton,\n'
        '} from "../nx-page-layout";\n'
    )
    ***REMOVED*** Only add if not already present
    if "nx-page-layout" not in src:
        ***REMOVED*** Find a good insertion point — after the last `from "lucide-react"` or
        ***REMOVED*** `from "next/link"` block
        insert_after = None
        ***REMOVED*** Try `from "next/link";\n`
        m = re.search(r'import Link from "next/link";\n', src)
        if m:
            insert_after = m.end()
        else:
            ***REMOVED*** Try the last `from "lucide-react";` import
            matches = list(re.finditer(r'from "lucide-react";\n', src))
            if matches:
                insert_after = matches[-1].end()
            else:
                ***REMOVED*** Fall back to after the first `from "..."` import
    ...
    ***REMOVED*** 3. Replace wrapper JSX.
    ***REMOVED***
    ***REMOVED*** Pattern: `<>` ... `<BackgroundScene />` ... `<Footer />` ... `</>`
    ***REMOVED*** We want to extract the body content between <main ...> and </main>
    ***REMOVED*** (or whatever the body is) and wrap it in <NxPageLayout>...</NxPageLayout>.

    ***REMOVED*** Most common pattern: extract content between <main ...> and </main>
    ***REMOVED*** but some pages have different structures. Let's handle the common case.

    ***REMOVED*** Remove the leading wrapper lines:
    ***REMOVED***   <>
    ***REMOVED***     <BackgroundScene />
    ***REMOVED***     <CustomCursor />
    ***REMOVED***     <ScrollProgressBar />
    ***REMOVED***     <Navbar />
    ***REMOVED***
    ...
    ***REMOVED*** Strip the BackgroundScene/CustomCursor/ScrollProgressBar/Navbar lines (in any order)
    ***REMOVED*** at the start of the return body
    src = re.sub(
        r'(\breturn\s*\(\s*\n\s*)<>\s*\n'
    ...
    ***REMOVED*** Strip the <main ...> opening tag — keep its children
    ***REMOVED*** Replace:  <main className="...">  →  (nothing, the body content stays)
    ***REMOVED*** But we need to be careful — some main tags have content we want to keep.
    ***REMOVED*** Strategy: replace <main className="..."> with empty, and </main> with empty,
    ***REMOVED*** since NxPageLayout already provides its own <main>.
    src = re.sub(
        r'<main className="[^"]*">\s*\n',
        '',
        src,
    ...
    ***REMOVED*** Strip the trailing Footer + ScrollToTop + </>  →  </NxPageLayout>
    src = re.sub(
        r'\n\s*<Footer\s*/>\s*\n\s*<ScrollToTop\s*/>\s*\n\s*</>',
        '\n    </NxPageLayout>',
        src,
    )
    ***REMOVED*** Also handle pattern where ScrollToTop was already removed at top
    src = re.sub(
        r'\n\s*<Footer\s*/>\s*\n\s*</>',
        '\n    </NxPageLayout>',
        src,
