...
***REMOVED*** Batch browser audit — takes page list as arg1 (comma-separated)
***REMOVED*** Each line of agent-browser output: result + "✓ Done" status. We use head -1 to get the actual value.
set -e
IFS=',' read -ra PAGES <<< "$1"
...
for page in "${PAGES[@]}"; do
  i=$((i+1))
  agent-browser eval "window.location.hash = '***REMOVED***$page';" 2>&1 > /dev/null
  agent-browser wait 2500 2>&1 > /dev/null
  ***REMOVED*** Get the actual title (first line of output, not the status)
  TITLE=$(agent-browser get title 2>&1 | head -1 | tr -d '"')
  ***REMOVED*** Check active section
