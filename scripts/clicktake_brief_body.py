...
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
***REMOVED*** 1. FONT REGISTRATION (Noto Sans SC for body, Sarasa Mono SC for code)
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
FONT_DIR = "/usr/share/fonts"

***REMOVED*** Sans-serif body + headings — DejaVu Sans (static, English-optimized, reliable in ReportLab)
...
***REMOVED*** Serif (used sparingly) — Noto Serif SC (static weights available)
...
***REMOVED*** Mono — Sarasa Mono SC (clean monospace with CJK + Latin)
...
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
***REMOVED*** 2. PALETTE — ClickTake brand (cyber-futurist dark)
***REMOVED*** Hand-tuned to extend ***REMOVED***FF53A9 magenta / ***REMOVED***9B3DFF purple / ***REMOVED***136DFF blue
***REMOVED*** while respecting cascade tier discipline (low-sat for large areas)
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
***REMOVED*** XL tier — page backgrounds
PAGE_BG       = colors.HexColor('***REMOVED***03000D')   ***REMOVED*** near-black, matches site
SECTION_BG    = colors.HexColor('***REMOVED***0A0418')   ***REMOVED*** elevated
***REMOVED*** L tier — surfaces
CARD_BG       = colors.HexColor('***REMOVED***14082A')   ***REMOVED*** deep purple card
TABLE_STRIPE  = colors.HexColor('***REMOVED***0F051C')   ***REMOVED*** alternating row
***REMOVED*** M tier — structural fills
HEADER_FILL   = colors.HexColor('***REMOVED***1A0B2E')   ***REMOVED*** table header
COVER_BLOCK   = colors.HexColor('***REMOVED***1F0B3A')   ***REMOVED*** callout bg
***REMOVED*** S tier — edges & icons
BORDER        = colors.HexColor('***REMOVED***3A1845')   ***REMOVED*** magenta-tinted low-sat border
ICON          = colors.HexColor('***REMOVED***FF53A9')   ***REMOVED*** full magenta for icons
***REMOVED*** XS tier — emphasis
ACCENT        = colors.HexColor('***REMOVED***FF53A9')   ***REMOVED*** ClickTake magenta
ACCENT_2      = colors.HexColor('***REMOVED***9B3DFF')   ***REMOVED*** ClickTake purple
ACCENT_3      = colors.HexColor('***REMOVED***136DFF')   ***REMOVED*** ClickTake blue
CYAN          = colors.HexColor('***REMOVED***00F0FF')   ***REMOVED*** data callout accent
***REMOVED*** Typography
TEXT_PRIMARY  = colors.HexColor('***REMOVED***F0EBF8')   ***REMOVED*** off-white, matches site dark mode
TEXT_BODY     = colors.HexColor('***REMOVED***D5D0E3')   ***REMOVED*** slightly dimmer for body
TEXT_MUTED    = colors.HexColor('***REMOVED***88879B')   ***REMOVED*** muted gray-purple
***REMOVED*** Semantic
SEM_SUCCESS   = colors.HexColor('***REMOVED***4ADE80')
SEM_WARNING   = colors.HexColor('***REMOVED***FBBF24')
SEM_ERROR     = colors.HexColor('***REMOVED***F87171')
SEM_INFO      = colors.HexColor('***REMOVED***60A5FA')
...
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
***REMOVED*** 4. PAGE FRAME — dark background, header rule, page number footer
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
PAGE_W, PAGE_H = A4
...
def draw_page_chrome(canvas_obj, doc):
    ...
    ***REMOVED*** Full-bleed dark background
    c.setFillColor(PAGE_BG)
    c.rect(0, 0, PAGE_W, PAGE_H, fill=1, stroke=0)

    ***REMOVED*** Subtle decorative grid (faint)
    ...
    ***REMOVED*** Top accent rule (gradient effect via 3 segments)
    ...
    ***REMOVED*** Page number (skip on TOC page = page 1)
    if doc.page > 1:
        c.setFont('Mono', 8.5)
        c.setFillColor(TEXT_MUTED)
        c.drawCentredString(PAGE_W / 2, bot_y - 14, str(doc.page))

    ***REMOVED*** Bottom-left mission tag
    c.setFont('Mono', 7.5)
    c.setFillColor(TEXT_MUTED)
    c.drawString(LEFT_M, bot_y - 14, 'SUPER Z · Z.AI')

    ***REMOVED*** Bottom-right date
    c.setFont('Mono', 7.5)
    c.setFillColor(TEXT_MUTED)
...
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
***REMOVED*** 5. CUSTOM FLOWABLES & HELPERS
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
...
def make_table(data, col_weights=None, header=True, hAlign='CENTER', first_col_left=True):
    ...
    ***REMOVED*** Wrap all cells in Paragraph
    wrapped = []
...
def code_block(code_text, language=None):
    ...
    ***REMOVED*** Escape XML special chars
    safe = code_text.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
    ***REMOVED*** Preserve newlines
    safe = safe.replace('\n', '<br/>')
    ***REMOVED*** Preserve leading spaces
    safe = re.sub(r'  ', '&nbsp;&nbsp;', safe)
    lines = []
    if language:
...
def bullet_list(items, style=S_BULLET):
    ...
    out = []
    for item in items:
        out.append(Paragraph(f'<font color="***REMOVED***FF53A9">▸</font>&nbsp;&nbsp;{item}', style))
    return out
...
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
***REMOVED*** 6. DOC TEMPLATE WITH TOC SUPPORT
***REMOVED*** ─────────────────────────────────────────────────────────────────────────────
...
***REMOVED*** ═════════════════════════════════════════════════════════════════════════════
***REMOVED*** 7. CONTENT — TOC + EXECUTIVE SUMMARY + 12 PARTS + 2 APPENDICES
***REMOVED*** ═════════════════════════════════════════════════════════════════════════════
...
def build_story():
    ...
    ***REMOVED*** ─── EXECUTIVE SUMMARY ───────────────────────────────────────────────────
    ...
    story.append(Paragraph(
        ...
        'bug reported in Elite Mode. This brief closes that gap. It spec&***REMOVED***64257;es a full transformation '
        ...
        'sacri&***REMOVED***64257;cing the GEO infrastructure that already puts the site ahead of 99% of the web.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'a site that needs a rebuild — they are the markers of a site that needs to be &***REMOVED***64257;nished.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        '17-18). Each phase has a clear definition-of-done, owner suggestion, and risk pro&***REMOVED***64257;le. '
        ...
        '3D experience on &***REMOVED***64257;ve pages) in 9 weeks and still deliver a visible transformation.',
        S_BODY))
    ...
    story.append(info_callout(
        'Reading guide',
        'Parts I-II cover SEO and GEO audits with prioritized, copy-paste-ready &***REMOVED***64257;xes. Part III '
    ...
    ***REMOVED*** ─── PART I — CHAPTER 1: SEO AUDIT — CURRENT STATE ─────────────────────
    ...
    story.append(Paragraph(
        ...
        'page-speci&***REMOVED***64257;c and async-capable, structured data spans seven schema.org types, and '
        ...
        'chapter documents the current state in detail; Chapter 2 prioritizes the &***REMOVED***64257;xes.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'property (tier 3 cities get priority 0.8, tier 2 get 0.6, tier 1 get 0.5) and of&***REMOVED***64257;ce '
    ...
    story.append(Paragraph('robots.txt — duplicate &***REMOVED***64257;le con&***REMOVED***64258;ict', S_H2))
    story.append(Paragraph(
        'Two robots &***REMOVED***64257;les coexist in the repository. The modern dynamic route '
        ...
        'the dynamic route — a con&***REMOVED***64258;ict acknowledged in a code comment in layout.tsx: '
        ...
        'workaround. This shadowing is the &***REMOVED***64257;rst thing to &***REMOVED***64257;x in Phase 0.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'geo.position, and ICBM in the other &***REMOVED***64257;eld. Six pages use async generateMetadata: '
        ...
        'and tag-speci&***REMOVED***64257;c keywords. Static metadata exports cover home, about, careers, '
    ...
    story.append(Paragraph(
        ...
        '<font face="Mono">src/components/site/json-ld.tsx</font> which exports &***REMOVED***64257;ve schema '
        ...
        'foundingDate, slogan, telephone, contactPoint array, address array for four of&***REMOVED***64257;ces, '
        'areaServed, sameAs, knowsAbout) plus four ProfessionalService/LocalBusiness blocks — one per '
        'of&***REMOVED***64257;ce in Birmingham, Multan, Austin, and Dubai — each with parentOrganization, '
    ...
    ***REMOVED*** ─── PART I — CHAPTER 2: SEO GAPS & PRIORITIZED FIXES ───────────────────
    story += section_opener('PART I · CHAPTER 2', 'SEO Audit — Gaps & Prioritized Fixes')

    story.append(Paragraph(
        'The eight gaps identi&***REMOVED***64257;ed in Chapter 1 are listed below in priority order — highest '
        'expected SEO lift per unit engineering effort &***REMOVED***64257;rst. Each gap includes the rationale, '
    ...
    story.append(Paragraph(
        ...
        'robots.txt instead of the restrictive, sitemap-declaring one. Worse, the static &***REMOVED***64257;le '
        'permits crawling of <font face="Mono">/api/</font> and <font face="Mono">/admin/</font>, '
        'which the dynamic &***REMOVED***64257;le correctly disallows. Expected lift: crawl budget optimization, '
        'admin panel de-indexation, sitemap discovery. Effort: S (single git rm).',
        S_BODY))
    story.append(code_block(
        'rm public/robots.txt\n***REMOVED*** Verify: curl https://clicktaketech.com/robots.txt\n***REMOVED*** Should now return the dynamic route output with Sitemap: declaration',
        'shell'))
    ...
    story.append(Paragraph(
        ...
        'of&***REMOVED***64257;take. Effort: S.',
        S_BODY))
    story.append(code_block(
    ...
    story.append(Paragraph(
        ...
        'bestRating/worstRating &***REMOVED***64257;elds) plus individual Review items per testimonial unlocks '
    ...
    story.append(Paragraph(
        ...
        'next/image with explicit width, height, and priority &***REMOVED***64258;ags. Lift: prevents CLS, '
...
***REMOVED*** ─── PART II — CHAPTER 3: GEO AUDIT ─────────────────────────────────────────
def add_part_ii_ch3(story):
    ...
    story.append(Paragraph(
        ...
        'piece is missing: the <font face="Mono">llms.txt</font> &***REMOVED***64257;le.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'server metadata. <font face="Mono">jwks.json</font> exposes JWT veri&***REMOVED***64257;cation keys. '
    ...
    story.append(Paragraph(
        ...
        'DNSSEC is con&***REMOVED***64257;gured with algorithm 13 (ECDSAP256SHA256); a DS record at the registrar '
    ...
    story.append(Paragraph(
        'The llms.txt spec, proposed by Jeremy Howard in 2024, de&***REMOVED***64257;nes a markdown-formatted '
        '/llms.txt &***REMOVED***64257;le at the site root that gives LLMs a curated, narrative overview of the '
        'site\'s content and offerings. It is the GEO equivalent of sitemap.xml for AI crawlers — '
        'and it is the one specifically-named GEO &***REMOVED***64257;le missing from ClickTake. The &***REMOVED***64257;le '
...
***REMOVED*** ─── PART II — CHAPTER 4: GEO IMPLEMENTATION ───────────────────────────────
def add_part_ii_ch4(story):
    ...
    story.append(Paragraph(
        'This chapter operationalizes the &***REMOVED***64257;ndings of Chapter 3. It contains the full draft '
    ...
    story.append(Paragraph(
        'The &***REMOVED***64257;le lives at <font face="Mono">public/llms.txt</font> and ships in Phase 0. It '
    ...
    story.append(Paragraph(
        ...
        'an "Export to llms-full.txt" button in /admin/cms that regenerates the &***REMOVED***64257;le from '
        'current CMS content).',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'increase AI citation of&***REMOVED***64257;take:',
        S_BODY))
    story += bullet_list([
    ...
    story.append(Paragraph(
        ...
        'free, privacy-&***REMOVED***64257;rst, no consent banner required, surfaces Core Web Vitals RUM and '
        ...
        'Overview &***REMOVED***64257;lters weekly for brand mentions, and runs manual prompts in ChatGPT '
...
***REMOVED*** ─── PART III — CHAPTER 5: ELITE MODE VISIBILITY FIX ────────────────────────
def add_part_iii(story):
    ...
    story.append(Paragraph(
        ...
        'has no Elite palette mapping at all. This chapter de&***REMOVED***64257;nes Elite Mode as a new '
        'premium visual tier layered on top of the existing 4-mode system, &***REMOVED***64257;xes the '
        'underlying token cascade, and speci&***REMOVED***64257;es the toggle UX.',
        S_BODY))

    story.append(Paragraph('Scope de&***REMOVED***64257;nition — what Elite Mode is', S_H2))
    story.append(Paragraph(
        'Elite Mode is a &***REMOVED***64257;fth palette variant — an elevated visual layer activated by an opt-'
        'in toggle (auth-gated for returning users, free for &***REMOVED***64257;rst 1000 signups as a loyalty '
        'reward). Visually, Elite Mode ampli&***REMOVED***64257;es the cyber-futurist palette: deeper 3D '
    ...
    story.append(Paragraph(
        'The admin panel CSS at <font face="Mono">src/app/admin/admin-globals.css</font> de&***REMOVED***64257;nes '
        ...
        'backgrounds, dialog overlays, and form &***REMOVED***64257;eld focus rings — are de&***REMOVED***64257;ned only '
        'in the dark variant and fall through to dark-palette tokens when the user is in light '
        'mode. The result: dark text on dark backgrounds, invisible form &***REMOVED***64257;elds, and '
    ...
    story.append(Paragraph('The &***REMOVED***64257;x — a uni&***REMOVED***64257;ed token cascade', S_H2))
    story.append(Paragraph(
        'The architectural &***REMOVED***64257;x is a uni&***REMOVED***64257;ed CSS custom-property cascade where Elite '
        'Mode is a &***REMOVED***64257;fth variant and every component rule reads from tokens rather than '
        ...
        '<font face="Mono">background: ***REMOVED***14082A</font>. This guarantees that switching the theme '
        'attribute on the html element updates every component without exception.',
        S_BODY))
    story.append(code_block(
        ':root {\n  /* Base palette — dark by default */\n  --bg:           ***REMOVED***03000D;\n  --bg-elev:      ***REMOVED***0A0418;\n  --surface-1:    ***REMOVED***14082A;\n  --surface-2:    ***REMOVED***1F0B3A;\n  --border:       ***REMOVED***3A1845;\n  --text:         ***REMOVED***F0EBF8;\n  --text-muted:   ***REMOVED***88879B;\n  --accent:       ***REMOVED***FF53A9;\n  --accent-2:     ***REMOVED***9B3DFF;\n  --accent-3:     ***REMOVED***136DFF;\n}\n\n:root[data-theme="light"] {\n  --bg:           ***REMOVED***FFFFFF;\n  --bg-elev:      ***REMOVED***FAFAFC;\n  --surface-1:    ***REMOVED***F4F2F8;\n  --surface-2:    ***REMOVED***EAE7F0;\n  --border:       ***REMOVED***D5D0E0;\n  --text:         ***REMOVED***0A0E1A;\n  --text-muted:   ***REMOVED***5A5870;\n  --accent:       ***REMOVED***E0197A;  /* slightly darker for AA contrast on white */\n  --accent-2:     ***REMOVED***7B2FD9;\n  --accent-3:     ***REMOVED***0F5BD9;\n}\n\n:root[data-theme="elite"] {\n  --bg:           ***REMOVED***02000B;  /* even deeper black */\n  --bg-elev:      ***REMOVED***08021A;\n  --surface-1:    ***REMOVED***190A35;  /* richer purple */\n  --surface-2:    ***REMOVED***260D4E;\n  --border:       ***REMOVED***5A1F70;  /* more visible border */\n  --text:         ***REMOVED***FAF7FF;  /* brighter text */\n  --text-muted:   ***REMOVED***A09BB5;\n  --accent:       ***REMOVED***FF6BB9;  /* luminous magenta */\n  --accent-2:     ***REMOVED***B066FF;  /* luminous purple */\n  --accent-3:     ***REMOVED***3D8BFF;  /* luminous blue */\n  --elite-glow:   0 0 24px rgba(255, 107, 185, 0.4);  /* exclusive Elite glow */\n}',
        'css'))
    ...
    story.append(Paragraph(
        ...
        'to a 5-mode segmented control — a horizontal pill with &***REMOVED***64257;ve segments, the Elite '
    ...
    story.append(info_callout(
        'Acceptance criteria for the Elite Mode &***REMOVED***64257;x',
        '1) Switching to light mode in admin panel shows all text, all form &***REMOVED***64257;elds, all '
...
***REMOVED*** ─── PART IV — CHAPTER 6: 3D BACKGROUND ENHANCEMENTS ───────────────────────
def add_part_iv(story):
    ...
    story.append(Paragraph(
        ...
        'renders &***REMOVED***64258;oating geometric shapes via CSS transforms), a Canvas2D background '
        ...
        '&***REMOVED***64258;oating icosahedron, and a 1200-particle &***REMOVED***64257;eld with mouse parallax). This '
        'chapter speci&***REMOVED***64257;es the upgrade to a proper @react-three/fiber + drei + postprocessing '
    ...
    story.append(Paragraph(
        'The postprocessing chain applies &***REMOVED***64257;ve effects in order: Bloom (intensity 0.8, '
    ...
    story.append(Paragraph(
        'The current 1200-particle CPU &***REMOVED***64257;eld is replaced by a GPU-instanced 5000-particle '
    ...
    story.append(Paragraph(
        ...
        'measurement over the &***REMOVED***64257;rst 60 frames after mount. The hook returns immediately '
        'with a preliminary tier, then re-&***REMOVED***64257;nes after measurement. The SceneProvider '
...
***REMOVED*** ─── PART V — CHAPTER 7: 3D CHARACTER SYSTEM ────────────────────────────────
def add_part_v(story):
    ...
    story.append(Paragraph(
        ...
        'chapter speci&***REMOVED***64257;es the full character roster, the rig spec, the animation set, the '
    ...
    story += bullet_list([
        ...
        '<b>Scroll-react</b> — character\'s pose changes as the user scrolls: at top of page the character is "looking up" (anticipatory), in middle "engaged" (working pose), at bottom "satis&***REMOVED***64257;ed" (gesture of completion). The pose blends smoothly across three scroll keypoints.',
    ...
    story.append(Paragraph(
        ...
        'synthesized via the z-ai-web-dev-sdk TTS endpoint and cached in IndexedDB on &***REMOVED***64257;rst '
...
***REMOVED*** ─── PART VI — CHAPTER 8: TEXT BOX REDESIGN ─────────────────────────────────
def add_part_vi(story):
    story += section_opener('PART VI · CHAPTER 8', 'Text Box Redesign — Futuristic 3D-Style UI Component System')

    story.append(Paragraph(
        'Every text-input and text-display component on the site gets a uni&***REMOVED***64257;ed redesign: '
    ...
    story.append(Paragraph(
        ...
        '<font face="Mono">transform: translateY(-2px)</font>. On focus, the border solidi&***REMOVED***64257;es '
        ...
        'this &***REMOVED***64257;eld" head turn. Depth is achieved via a parent '
        ...
        '3D extrusion that reads as "this &***REMOVED***64257;eld is part of the 3D world, not pasted on top '
        'of it".',
        S_BODY))
    ...
    story.append(make_table([
        ...
        ['Focus', 'rgba(255,83,169,0.08)', '***REMOVED***FF53A9 1.5px', 'translateY(-2px) translateZ(12px)', 'box-shadow: 0 0 0 3px rgba(255,83,169,0.2)'],
        ['Error', 'rgba(248,113,113,0.08)', '***REMOVED***F87171 1.5px', 'translateX(0)', 'Shake animation 200ms'],
    ...
    story.append(Paragraph(
        ...
        'First, the label floats: when the &***REMOVED***64257;eld is empty, the label sits inside the &***REMOVED***64257;eld '
        'as placeholder text; on focus or when &***REMOVED***64257;lled, the label slides up 12px and shrinks '
        ...
        'on focus, displaying "0 / 500" in mono font with a magenta progress arc that &***REMOVED***64257;lls '
        'as the user types. Third, the validation checkmark: when a &***REMOVED***64257;eld passes async '
    ...
    story.append(Paragraph(
        ...
        '<font face="Mono">prefers-reduced-motion</font> — instead of shaking, the &***REMOVED***64257;eld '
        'shows a 200ms red border pulse. Keyboard navigation: Tab moves between &***REMOVED***64257;elds, '
        'Enter submits forms, Esc clears the current &***REMOVED***64257;eld. Screen readers announce label, '
    ...
    story.append(Paragraph(
        ...
        'with translateZ), the focus-glow box-shadow, and the &***REMOVED***64258;oating-label animation. The '
    ...
    story.append(info_callout(
        ...
        '***REMOVED***FF53A9 · Glow 0 0 0 3px rgba(255,83,169,0.2) · Label-float 14px→11px, 0→-12px · Counter '
        'Mono 11px rgba(255,83,169,1) · Check 16px ***REMOVED***FF53A9 spring 200ms · Padding 12px 16px · '
        'Radius 8px · Font NotoSans 14px/20px · Touch-target 44×44px min · Focus-ring 2px ***REMOVED***FF53A9 '
        '+ 3px glow.',
        SEM_INFO))
...
***REMOVED*** ─── PART VII — CHAPTER 9: IMMERSIVE STORYTELLING ───────────────────────────
def add_part_vii(story):
    ...
    story.append(Paragraph(
        ...
        'ClickTake City — and every page is a district with its own guide. This chapter de&***REMOVED***64257;nes '
    ...
    story.append(Paragraph(
        ...
        '1.0 seconds, with the mesh opacity rising from 0 to 1 over the &***REMOVED***64257;nal 0.4 seconds. '
    ...
    story.append(Paragraph(
        ...
        'section in view. At the bottom (satis&***REMOVED***64257;ed), the character turns back to the visitor '
        ...
        'are uni&***REMOVED***64257;ed, not duplicated.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'synthesized via the z-ai-web-dev-sdk TTS endpoint on &***REMOVED***64257;rst visit and cached in '
    ...
    story.append(Paragraph('Narrative &***REMOVED***64258;ow diagram (text representation)', S_H2))
    story.append(code_block(
...
***REMOVED*** ─── PART VIII — CHAPTER 10: IMPLEMENTATION ROADMAP ────────────────────────
def add_part_viii(story):
    ...
    story.append(Paragraph(
        ...
        'suggestion, a de&***REMOVED***64257;nition-of-done, and a risk pro&***REMOVED***64257;le. Phase gates allow '
        'pause points if team bandwidth tightens — a smaller team can ship Phases 0-3 (SEO wins '
        'plus the hero 3D experience on &***REMOVED***64257;ve pages) in 9 weeks and still deliver a visible '
    ...
    story.append(Paragraph(
        ...
        'migrates admin @import fonts to next/font. Veri&***REMOVED***64257;cation: Google Rich Results Test '
        'passes on all 5 modi&***REMOVED***64257;ed pages, Schema.org validator 100% pass, Vercel Analytics '
    ...
    story.append(Paragraph(
        ...
        'cascade from Chapter 5 and upgrade theme-toggle to 5-mode segmented control. De&***REMOVED***64257;nition-of-done: '
        'Lighthouse CI passes on a &***REMOVED***64257;ve-page subset, 3D bundle ≤ 280 KB gzipped, Elite Mode '
    ...
    story.append(Paragraph(
        ...
        'handlers. De&***REMOVED***64257;nition-of-done: all 3 characters render at 60fps on M1 baseline, '
    ...
    story.append(Paragraph(
        ...
        'voice-over on 3 hero pages (Home, Services, About). De&***REMOVED***64257;nition-of-done: 10 pages '
    ...
    story.append(Paragraph(
        ...
        'Benchmark, Client Metrics Dashboard). De&***REMOVED***64257;nition-of-done: every form uses NxInput '
    ...
    story.append(Paragraph(
        ...
        'localStorage, Elite unlock modal. De&***REMOVED***64257;nition-of-done: full storytelling journey '
        ...
        '&***REMOVED***64257;rst play and only re-synthesizing if the line text changes.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'unlock. De&***REMOVED***64257;nition-of-done: Lighthouse CI green on all routes, pa11y-ci green on '
        ...
        'de&***REMOVED***64257;ning MVP as visual layer only and deferring premium content to v2.',
        S_BODY))

    story.append(Paragraph('Phase 7 — Launch (weeks 17-18)', S_H2))
    story.append(Paragraph(
        'Staged rollout: 10% of traffic in week 17 (Vercel Edge Config &***REMOVED***64258;ag), 50% mid-week, '
        ...
        'end of week 18. De&***REMOVED***64257;nition-of-done: 100% rollout, GEO dashboard showing baseline '
...
***REMOVED*** ─── PART IX — CHAPTER 11: KPIs & MEASUREMENT ───────────────────────────────
def add_part_ix(story):
    ...
    story.append(Paragraph(
        ...
        'Phase 0 — free, privacy-&***REMOVED***64257;rst, no consent banner required. Surfaces pageview '
        'counts, ai.* referral sources, and Core Web Vitals RUM. Second, PostHog installs in '
        'Phase 1 for product analytics (session replay, funnel analysis, feature &***REMOVED***64258;ags '
...
***REMOVED*** ─── PART X — CHAPTER 12: RISK REGISTER ────────────────────────────────────
def add_part_x(story):
    ...
    story.append(make_table([
        ['***REMOVED***', 'Risk', 'Prob', 'Impact', 'Mitigation'],
...
***REMOVED*** ─── APPENDIX A: QUICK-WIN CODE PATCHES ────────────────────────────────────
def add_appendix_a(story):
    story += section_opener('APPENDIX A', 'Quick-Win Code Patches')

    story.append(Paragraph(
        'Copy-paste-ready patches for the 7 Phase 0 SEO/GEO quick wins. Each patch has a &***REMOVED***64257;le '
        'path, the code, and a veri&***REMOVED***64257;cation command. All patches are designed to ship as a '
        'single PR in week 1.',
        S_BODY))
    ...
    story.append(Paragraph(
        ...
        'Chapter 4. Veri&***REMOVED***64257;cation: <font face="Mono">curl https://clicktaketech.com/llms.txt</font> '
        'returns the markdown.',
        S_BODY))

    story.append(Paragraph('A.2 — Delete public/robots.txt', S_H3))
    story.append(code_block(
        'git rm public/robots.txt\n***REMOVED*** Verify\ncurl -sI https://clicktaketech.com/robots.txt | head -5\n***REMOVED*** Should return Content-Type: text/plain with the dynamic robots.ts output\n***REMOVED*** Body should include "Sitemap: https://clicktaketech.com/sitemap.xml"',
        'shell'))

    story.append(Paragraph('A.3 — RSS feed at /feed.xml', S_H3))
    story.append(Paragraph(
        'Full code in Gap 5 of Chapter 2. Create '
        '<font face="Mono">src/app/feed.xml/route.ts</font>. Veri&***REMOVED***64257;cation: '
    ...
    story.append(Paragraph(
        ...
        'the NxFaq component. Full code in Gap 2 of Chapter 2. Veri&***REMOVED***64257;cation: Google Rich '
    ...
    story.append(Paragraph(
        ...
        'element (replacing the current class-based .dark / .theme-custom system), de&***REMOVED***64257;ne '
...
***REMOVED*** ─── APPENDIX B: GLOSSARY & REFERENCES ─────────────────────────────────────
def add_appendix_b(story):
    ...
    story.append(make_table([
        ...
        ['Vercel Analytics', 'Privacy-&***REMOVED***64257;rst, cookie-less analytics built into Vercel. Free for hobby tier; no consent banner required.'],
        ['PostHog', 'Open-source product analytics with session replay, feature &***REMOVED***64258;ags, and A/B testing. Recommended for Phase 1 install.'],
    ...
    story.append(Paragraph(
        '<font face="Mono" color="***REMOVED***88879B" size="7.5">CLICKTAKE TECHNOLOGIES · 3D STORYTELLING PLATFORM BRIEF · v1.0 · JULY 2026 · PREPARED BY SUPER Z · Z.AI</font>',
