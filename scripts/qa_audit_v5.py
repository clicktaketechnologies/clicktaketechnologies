...
***REMOVED*** ============== 1. DISCOVER ALL PAGES ==============
***REMOVED*** data-page="..." on <section> tags (not inside JS template strings)
***REMOVED*** Match: <section ... data-page="..."
section_pages = set()
...
***REMOVED*** ============== 2. PAGES MAP in SPA router ==============
***REMOVED*** Look for: const PAGES = { ... }; (closing brace may be indented)
...
***REMOVED*** ============== 3. PER-PAGE SEO METADATA IN PAGES MAP ==============
if pages_map_match:
    ***REMOVED*** Parse each entry to verify it has title, desc, kw, url (PAGES map field names)
    ***REMOVED*** Split on top-level entries
...
***REMOVED*** ============== 4. INTERNAL LINKS — every internal href resolves to a page ==============
***REMOVED*** href="***REMOVED***page-slug" or onclick="navigateTo('slug')"
hash_links = re.findall(r'href="***REMOVED***([^"]+)"', SRC)
nav_calls = re.findall(r"navigateTo\(['\"]([^'\"]+)['\"]\)", SRC)
all_internal = set(hash_links + nav_calls)
***REMOVED*** Exclude skip-link target and empty strings
...
***REMOVED*** ============== 5. GLOBAL ELEMENTS PRESENT ONCE ==============
global_checks = [
...
***REMOVED*** ============== 6. PER-PAGE STRUCTURAL CHECKS ==============
***REMOVED*** For each page section, check it has: hero, breadcrumb (except home), CTA section, glass tokens
page_section_pattern = re.compile(
    r'<section[^>]*\sdata-page="([^"]+)"[^>]*>(.*?)</section>\s*(?=<section[^>]*\sdata-page="|<!--\s*END\s+PAGES\s*-->|<footer)',
    re.S
)

***REMOVED*** Better: split by data-page attribute and capture until next section
***REMOVED*** Use simpler approach: find each section's body
page_bodies = {}
for m in re.finditer(r'<section[^>]*\sdata-page="([^"]+)"[^>]*>', SRC):
    slug = m.group(1)
    start = m.end()
    ***REMOVED*** Find next <section data-page= or end marker
...
for slug, body in page_bodies.items():
    ***REMOVED*** Hero = presence of H1 (sub-pages) or hero class (home)
    ...
    if slug == "home":
        ...
        ***REMOVED*** Sub-pages: hero is the pt-32 lg:pt-40 region with h1
        if not has_h1:
            pages_missing_hero.append(slug)
        ***REMOVED*** Sub-pages should have breadcrumb
        if "breadcrumb" not in body.lower() and "crumb" not in body.lower():
            pages_missing_breadcrumb.append(slug)
    ***REMOVED*** Glass tokens
    ...
    ***REMOVED*** Legacy markup checks (old classes from v3/v4)
...
for val, desc in placeholders:
    if val.lower() == "example.com":
        ***REMOVED*** skip clicktaketech.com etc
...
***REMOVED*** ============== 14. PALETTE COMPLIANCE ==============
PALETTE = {"***REMOVED***03000D", "***REMOVED***070018", "***REMOVED***0D0025", "***REMOVED***136DFF", "***REMOVED***FF53A9", "***REMOVED***7B2FBE", "***REMOVED***F0EBF8", "***REMOVED***9A8CB5"}
PALETTE_LOWER = {c.lower().lstrip('***REMOVED***') for c in PALETTE}
***REMOVED*** Acceptable off-palette: WhatsApp brand greens, skin tones for mascots, tints/shades for subtle gradients
ACCEPTABLE_OFF_PALETTE = {
    "128c7e", "25d366",  ***REMOVED*** WhatsApp brand greens
    "c98a66", "e8a884", "ffb4dc", "ffe0c2",  ***REMOVED*** mascot skin tones / illustration tints
    "0a0118", "0a1746",  ***REMOVED*** darker background variants for depth
    "e0197a",  ***REMOVED*** pink accent variant
    "f4eefb", "d4b4ff", "b4d1ff",  ***REMOVED*** light tints of brand colors used for subtle backgrounds
}
...
***REMOVED*** ============== 15. EXTERNAL RESOURCE HEALTH ==============
...
***REMOVED*** External scripts should have defer/async
...
***REMOVED*** ============== 16. DUPLICATE PAGE SLUGS ==============
