...
# Collect ALL import sources across src/
import_content = ""
...
for fn in sorted(os.listdir(SITE_DIR)):
    if not fn.endswith(".tsx"):
        continue
    comp_dir = os.path.join(SITE_DIR, fn, "__init__")  # placeholder
    name = fn.replace(".tsx", "")
    # Check if anything imports this file (by path stem)
    patterns = [
        f'from "./{name}"',
        f'from "../{name}"',
    ...
    imported = False
    for p in patterns:
        # crude check: count occurrences, subtract self-occurrence
