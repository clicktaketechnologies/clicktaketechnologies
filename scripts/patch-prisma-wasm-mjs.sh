***REMOVED***!/bin/bash
***REMOVED*** Postinstall patch for @prisma/client — create missing wasm.mjs shim.
***REMOVED***
***REMOVED*** Prisma 6.19.3's package.json exports map for `./wasm` points `import` and
***REMOVED*** `default` to `wasm.mjs`, but only `wasm.js` (CommonJS) is shipped. This
***REMOVED*** causes Turbopack/Next.js builds to fail with "Module not found: Can't
***REMOVED*** resolve '@prisma/client/wasm'" when db.ts imports from `@prisma/client/wasm`.
***REMOVED***
***REMOVED*** This shim re-exports the CommonJS wasm.js as ESM, so the import resolves.
