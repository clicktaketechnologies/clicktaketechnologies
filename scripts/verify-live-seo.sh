***REMOVED***!/bin/bash
***REMOVED*** Live SEO verification — confirms the P0+P1 fixes from the technical SEO
***REMOVED*** audit are actually live on clicktaketech.com. Pure read-only HTTP probing,
***REMOVED*** no auth required.
***REMOVED***
***REMOVED*** Run: bash /home/z/my-project/scripts/verify-live-seo.sh
...
check() {
  ...
  ***REMOVED*** Case-insensitive substring match — both sides lowercased.
...
***REMOVED*** Sitemap URL must be apex, not www
...
***REMOVED*** robots.txt host + sitemap
...
***REMOVED*** Verify the image itself is reachable
...
***REMOVED*** Homepage <title> should NOT have two "ClickTake Technologies" repeats
