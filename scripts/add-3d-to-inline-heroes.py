...
***REMOVED*** Floating character block — absolute positioned, lg-only, doesn't block clicks
...
def process_file(path: Path, variant: str) -> bool:
    ...
    ***REMOVED*** Skip if already has 3D character
    if "<Nx3DCharacter" in content:
        print(f"  - Already has 3D character")
        return False

    ***REMOVED*** Find first <NxPageLayout> opening tag
    ...
    insert_pos = m.end()
    ***REMOVED*** Build the block for this variant
