...
def add_character_prop(content: str, variant: str) -> str:
    ...
    if 'character="' in content:
        ***REMOVED*** Already has a character prop somewhere — skip (avoid double-add)
        ***REMOVED*** But only skip if it's on the FIRST NxPageHero. For safety, just skip.
        return content

    ***REMOVED*** Find first <NxPageHero occurrence
    m = re.search(r'<NxPageHero\s*\n', content)
    if not m:
        ***REMOVED*** Try same-line variant
    ...
    insert_pos = m.end()
    ***REMOVED*** We want to insert after the tag name. m.end() is right after `<NxPageHero\n` or `<NxPageHero `
    ***REMOVED*** Insert the character prop on its own line, preserving indentation by looking at next line
    next_line_end = content.find('\n', insert_pos)
    if next_line_end == -1:
        next_line_end = len(content)
    next_line = content[insert_pos:next_line_end]
    ***REMOVED*** Detect indentation from the next attribute (typically 8 spaces)
...
def add_scene_to_first_section(content: str) -> str:
    ...
    ***REMOVED*** Find first <NxPageSection ... > opening tag and its closing >
    ***REMOVED*** Then find its matching children and closing </NxPageSection>
    m = re.search(r'<NxPageSection\b([^>]*)>', content)
    if not m:
        return content

    attrs = m.group(1)
    ***REMOVED*** Make sure 'relative' is in className — if not, add it
    if 'className=' in attrs:
        ***REMOVED*** Append " relative" before the closing quote
        ***REMOVED*** Handle both ' and "
        if '"' in attrs.split('className=')[1][:1] or True:
            ***REMOVED*** Try double-quote style
            new_attrs = re.sub(
                r'(className=")([^"]*)(")',
                r'\1\2 relative\3',
                attrs,
            ...
            ***REMOVED*** If no change happened (className uses single quote), fall through
            if new_attrs == attrs:
                new_attrs = re.sub(
                    r"(className=')([^']*)(')",
                    r"\1\2 relative\3",
        ...
        ***REMOVED*** Add className="relative" before the final >
        new_attrs = attrs.rstrip() + ' className="relative"'

    ***REMOVED*** Rebuild the opening tag
    new_opening = f'<NxPageSection{new_attrs}>'
    content_after_open = content[m.end():]

    ***REMOVED*** Insert Nx3DScene + opening wrapper div as first children
    scene_block = (
        '\n        {/* 3D floating accents */}\n'
        '        <Nx3DScene density="low" />\n'
        '        <div className="relative z-10">\n'
    )
    ***REMOVED*** Find matching </NxPageSection>
    ***REMOVED*** Note: sections don't nest in these files (verified), so first close = match
    close_match = re.search(r'</NxPageSection>', content_after_open)
    if not close_match:
        return content  ***REMOVED*** malformed; skip

    children = content_after_open[:close_match.start()]
    after_close = content_after_open[close_match.start():]

    ***REMOVED*** Indent existing children by 2 spaces to fit inside wrapper div
    ***REMOVED*** Actually — they're already at 8-space indent. Adding wrapper at 8 spaces
    ***REMOVED*** means children should be at 10 spaces. But to keep things simple and not
    ***REMOVED*** break JSX text, we'll just close the wrapper before </NxPageSection>.
...
def add_import(content: str) -> str:
    ...
    if "Nx3DScene" in content:
        return content
    ***REMOVED*** Find the nx-page-layout import block
    m = re.search(
        r'import\s*\{([^}]+)\}\s*from\s*"../nx-page-layout"',
        content,
    )
    if m:
        ***REMOVED*** Add Nx3DScene to the named imports
        imports = m.group(1)
        if "Nx3DScene" not in imports:
            new_imports = imports.rstrip() + ", Nx3DScene"
            ***REMOVED*** Tidy up: ensure comma + space
            new_imports = re.sub(r',\s*,', ', ', new_imports)
            new_import_block = f'import{{{new_imports}}} from "../nx-page-layout"'
            ***REMOVED*** The regex match captured `import{...}from "..."` without spaces —
            ***REMOVED*** but the actual source has spaces. We need to do a direct
            ***REMOVED*** string replacement of the original match instead.
            return content.replace(m.group(0), new_import_block, 1)
        return content

    ***REMOVED*** No nx-page-layout import — add standalone import after last import line
    lines = content.split('\n')
    last_import_idx = -1
...
def process_file(path: Path, variant: str) -> bool:
    ...
    ***REMOVED*** Skip add_scene_to_first_section for now — many sections have
    ***REMOVED*** conditional layouts that would break with a wrapper div. The hero
    ***REMOVED*** already has the scene via NxPageHero's scene prop.
    ***REMOVED*** content = add_scene_to_first_section(content)
