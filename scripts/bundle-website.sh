...
***REMOVED*** Bundle the ClickTake website source into a clean zip for the user.
***REMOVED*** Excludes node_modules, .next, .env, .git, tool-results, worklog, etc.
...
***REMOVED*** Files and folders to include (whitelist approach for safety)
INCLUDE=(
  "src"
  "public"
  "prisma"
...
***REMOVED*** Add a README with quick start instructions
cat > "$STAGE/clicktake-website/README.md" <<'EOF'
***REMOVED*** ClickTake Technologies — Website
...
npm install        ***REMOVED*** or: bun install

***REMOVED*** 2. Create .env.local with:
***REMOVED***    DATABASE_URL="file:./db/custom.db"
***REMOVED***    (plus RESEND_API_KEY, TURNSTILE_SECRET_KEY, TURNSTILE_SITE_KEY if you wire them up)

***REMOVED*** 3. Run the dev server
npm run dev        ***REMOVED*** or: bun dev
```
...
***REMOVED*** Create a default .env.example (NOT .env — that should stay gitignored)
cat > "$STAGE/clicktake-website/.env.example" <<'EOF'
***REMOVED*** Database (SQLite for dev)
DATABASE_URL="file:./db/custom.db"

***REMOVED*** Email (Resend)
RESEND_API_KEY=""
CONTACT_TO_EMAIL="info@clicktaketech.com"
CONTACT_FROM_EMAIL="noreply@clicktaketech.com"

***REMOVED*** Cloudflare Turnstile (anti-bot)
