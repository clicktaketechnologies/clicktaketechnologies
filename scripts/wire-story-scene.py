...
***REMOVED*** Marker for the existing 3DCharacter wrapper div — used as insertion anchor.
ANCHOR = '<div className="pointer-events-none absolute right-0 top-24'

***REMOVED*** Import line to add if missing.
IMPORT_LINE = 'import { NxStoryScene } from "../nx-story-scene";'

***REMOVED*** Comment to insert above the new <NxStoryScene />
...
def patch_custom_hero(path: Path, variant: str) -> str:
    ...
    ***REMOVED*** 1) Add import if not present.
    if "NxStoryScene" not in src:
        ***REMOVED*** Insert after the Nx3DCharacter import line (consistent location).
        m = re.search(r'^(import \{ Nx3DCharacter \} from "\.\./nx-3d-character";)\s*$', src, flags=re.MULTILINE)
        if m:
            src = src[:m.end()] + "\n" + IMPORT_LINE + src[m.end():]
        else:
            ***REMOVED*** Fallback: add after first nx-3d-scene import.
    ...
    ***REMOVED*** 2) Insert <NxStoryScene .../> before the 3DCharacter wrapper div.
    ***REMOVED***    Only insert once.
    if f'<NxStoryScene variant="{variant}" />' not in src:
        ***REMOVED*** The anchor appears once per file.
        idx = src.find(ANCHOR)
        if idx == -1:
            return "FAIL: could not find anchor"
        ***REMOVED*** Find start of the line (preserve indentation).
        line_start = src.rfind("\n", 0, idx) + 1
        indent = src[line_start:idx]
        ***REMOVED*** Build the new block with the same indent.
        block = (
            f"{{/* Per-page 3D Story Scene — {variant} variant. Sits behind hero content. */}}\n"
...
***REMOVED*** ─── NxPageHero prop injection ───────────────────────────────────────────────
HERO_PROP_PAGES = [
...
def patch_hero_prop(path: Path, variant: str) -> str:
    ...
    ***REMOVED*** Find <NxPageHero ...> opening tag and inject storyVariant prop.
    ***REMOVED*** The opening tag ends with either '>' (self-style with children) or '/>' (rare).
    ***REMOVED*** We'll inject right before the first '>' that closes the opening tag.
    m = re.search(r'<NxPageHero\b([^>]*?)>', src, flags=re.DOTALL)
    if not m:
        return "FAIL: no <NxPageHero> tag"
    attrs = m.group(1)
    ***REMOVED*** Inject before the closing '>'. Preserve trailing whitespace/newline if any.
    new_attrs = attrs.rstrip()
    if new_attrs and not new_attrs.endswith("\n"):
        ***REMOVED*** Keep on same line if attrs are inline.
